    ...
    <div>
      <div style={{marginBottom:'1.5rem'}}>
        <div style={{display:'flex',alignItems:'baseline',gap:12,flexWrap:'wrap'}}>
          <h1 style={{fontFamily:"'Fraunces',serif",fontSize:'2rem',fontWeight:300,fontStyle:'italic',color:'var(--text)',letterSpacing:'-0.02em'}}>Daily Protocol</h1>
        ...
        <p style={{fontSize:11,color:'var(--muted)',letterSpacing:'0.06em',textTransform:'uppercase',marginTop:4}}>
          Blueprint Health — {new Date().toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric'})}
        </p>
      </div>
        ...
        {DAYS.map(day => {
          ...
          return (
            <button key={day} onClick={()=>setActiveDay(day)}
              ...
              <span style={{fontSize:10,color:active?'var(--accent)':'var(--muted2)'}}>{d===t&&t>0?'✓':d+'/'+t}</span>
            </button>
          );
        })}
      </div>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'1rem',flexWrap:'wrap',gap:12}}>
        <div>
          <div style={{fontFamily:"'Fraunces',serif",fontSize:'1.4rem',fontWeight:300,fontStyle:'italic',color:'var(--text)'}}>
            {activeDay} — {workout || 'Rest & Recovery'}
          </div>
        </div>
        <div style={{display:'flex',alignItems:'center',gap:12}}>
          <div style={{textAlign:'right'}}>
            <div style={{fontSize:11,color:'var(--muted)',letterSpacing:'0.06em',textTransform:'uppercase',marginBottom:4}}>{done} / {total} done</div>
          ...
          <button onClick={()=>resetDay(activeDay)}
            style={{background:'none',border:'1px solid var(--border)',borderRadius:99,padding:'5px 12px',
              fontSize:10,color:'var(--muted)',letterSpacing:'0.06em',textTransform:'uppercase',fontFamily:'inherit'}}>
            Reset
          </button>
        </div>
      </div>
      {sections.map((section, si) => {
        ...
        for (let i = 0; i < si; i++) taskIndexOffset += sections[i].tasks.length;
        return (
              ...
              return (
                <div key={idx} onClick={()=>toggle(activeDay, idx)}
                  ...
                  <div style={{width:17,height:17,borderRadius:'50%',flexShrink:0,marginTop:2,
                    ...
                    {isDone && <svg width="8" height="7" viewBox="0 0 9 7" fill="none">
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{display:'flex',alignItems:'baseline',gap:6,flexWrap:'wrap',marginBottom:3}}>
                      <span style={{fontSize:13,color:'var(--text)',lineHeight:1.4,textDecoration:isDone?'line-through':'none'}}>{task.title}</span>
                      {task.tags.map(tag => {
                        const s = TAG_STYLES[tag] || {};
                        return <span key={tag} style={{fontSize:10,padding:'1px 7px',borderRadius:99,whiteSpace:'nowrap',
